# Тестування працездатності системи

*У цьому розділі демонструється запити та результати тестування розробленого ПЗ за допомогою додатку Postman - HTTP клієнта для тестування серверного API.*

## Вікно керуючого термінала сервера

![Alt-text](./server-work.png)

## Відображення всіх сутностей

![Alt text](./task-list.png)

## Додавання сутності (CREATE)

### Запит

![Alt text](./create-task.png)

### Результат

![Alt text](./result-create.png)

## Зчитування сутності (READ)

### Результат

![Alt text](./read-task.png)

## Оновлення сутності (UPDATE)

### Запит

![Alt text](./update-task.png)

### Результат

![Alt text](./result-update.png)

## Видалення сутності (DELETE)

### Початковий стан списку

![Alt text](./init-delete.png)

### Запит

![Alt text](./delete-task.png)

### Результат

![Alt text](./result-delete.png)
